...
function Leads({leads,leadsDB,tasks,tasksDB,users,agents,currentUser,settings}) {
  const [tab,setTab]=useState("GCL"); const [showAdd,setShowAdd]=useState(false);
  const [sel,setSel]=useState(null); const [noteText,setNoteText]=useState(""); const [noteType,setNoteType]=useState("Call");
  const sources = settings?.lead_sources ? JSON.parse(settings.lead_sources) : LEAD_SOURCES_DEFAULT;
  const EF={name:"",phone:"",email:"",country:"🇬🇧 UK",source:sources[0]||"Other",branch:currentUser.branch,type:"B2C",agent_id:"",ielts_score:"",intake_target:""};
  const [form,setForm]=useState(EF);
  const filtered=useMemo(()=>leads.filter(l=>{
    if(l.lost)return false; if(l.list!==tab)return false;
            ...
            {filtered.map(lead=>{
              ...
              return (
                <tr key={lead.id}>
                  ...
                  <td style={S.td}><span style={{fontSize:12,color:days>2&&lead.list!=="ACL"?"#dc2626":"#37474f"}}>{lead.last_contact}{days>2&&lead.list!=="ACL"&&<div style={{fontSize:10,color:"#dc2626",fontWeight:700}}>⚠️ {days}d ago</div>}</span></td>
...
export default function App() {
  ...
  useEffect(()=>{
    if(authUser&&usersDB.data.length>0){const p=usersDB.data.find(u=>u.id===authUser.id);if(p)setCurrentUser(p);}
